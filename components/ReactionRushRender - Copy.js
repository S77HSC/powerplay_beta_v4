...
// ---------- 3D-ish PITCH (bevels + stadium light) ----------
function drawPitch(ctx, W, H) {
  const inset = 22;

  // grass stripes (slightly transparent so logo glow comes through)
  const stripeH = 28;
  ...
  light.addColorStop(0.0, 'rgba(255,255,255,0.10)');
  light.addColorStop(0.4, 'rgba(255,255,255,0.00)');
  light.addColorStop(1.0, 'rgba(0,0,0,0.20)');
  ctx.fillStyle = light; ctx.fillRect(0,0,W,H);

  // inner bevels (sunken field feel)
  ctx.save();
  ...
  if (logoOverlay && logoOverlay.complete && logoOverlay.naturalWidth) {
    ...
    ctx.font = '900 80px Inter, system-ui, sans-serif'; ctx.fillText('POWERPLAY', W/2, H * 0.16);
  }
  ctx.restore();

  // outer drop shadow (raised pitch feel)
  ctx.save();
...
function drawCornerLogo(ctx, W, H, now) {
  const { rrIcon } = ensureSprites();
  const pad = 10, size = 36;
  const x = W - pad - size / 2, y = H - pad - size / 2;

  ctx.save();
  const halo = ctx.createRadialGradient(x, y, 0, x, y, 36);
  halo.addColorStop(0, 'rgba(59,130,246,0.55)'); halo.addColorStop(1, 'rgba(59,130,246,0)');
  ctx.globalAlpha = 0.85; ctx.fillStyle = halo;
  ctx.beginPath(); ctx.arc(x, y, 30, 0, Math.PI*2); ctx.fill();

  ctx.globalAlpha = 0.6; ctx.lineWidth = 1.5; ctx.strokeStyle = '#60a5fa';
  const arcR = 30; const t = now * 0.004;
  ctx.beginPath(); for (let i=0;i<2;i++){ const a0 = t + i*Math.PI; const a1 = a0 + Math.PI*0.5; ctx.arc(x, y, arcR, a0, a1); } ctx.stroke();

  ctx.globalAlpha = 0.95;
  if (rrIcon && rrIcon.complete && rrIcon.naturalWidth) {
    ctx.drawImage(rrIcon, x - size/2, y - size/2, size, size);
  } else {
    ctx.fillStyle = '#3b82f6'; ctx.beginPath(); ctx.arc(x, y, size/2, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = '#0b1020'; ctx.font = '900 16px Inter, system-ui, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('RR', x, y);
  }
  ctx.restore();
}

// ---------- main (adds a skew/scaled world transform for 3D camera) ----------
export function drawWorld(canvas, dims, g, p, opts = {}) {
  if (!canvas) return;
  ...
  // clear & background (not tilted – feels like stadium backdrop)
  ctx.clearRect(0, 0, W, H);
  drawBackgroundLogo(ctx, W, H, now);

  // WORLD TRANSFORM (tilt + slight vertical scale + lift)
  ctx.save();
  if (view3D) {
    const tiltY = -0.22;   // shear on Y for camera pitch
    const scaleY = 0.90;   // slight vertical compression
    const liftY  = -H * 0.08; // lift to keep within frame
    ctx.transform(1, tiltY, 0, scaleY, 0, liftY);

    // optional mild camera follow on Y (adds life without breaking UI)
    const follow = p ? clamp((p.y - H*0.5) * -0.04, -H*0.06, H*0.06) : 0;
    ctx.translate(0, follow);
  }

  // pitch, border pulse and world
  drawPitch(ctx, W, H);
  ...
  // soft ground shadow under player / defenders (reads better with tilt)
  if (p) {
    ctx.save();
  ...
  // defenders shadows + actors
  for (const d of g.defenders) {
    ctx.save(); ctx.globalAlpha = 0.20; ctx.fillStyle = 'rgba(0,0,0,0.35)';
    ctx.beginPath(); ctx.ellipse(d.x, d.y + 7, d.r * 1.3, d.r * 0.6, 0, 0, Math.PI * 2); ctx.fill(); ctx.restore();
    drawDefender(ctx, d);
  }

  // trail, player, zones, fx, texts, goal
  drawTrail(ctx, g);
  drawPlayer(ctx, p, g);
  drawZones(ctx, g.zones || []);
  drawParticles(ctx, g);
