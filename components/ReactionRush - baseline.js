...
// Medkits
const MEDKIT_MIN = 12000;
const MEDKIT_VAR = 10000;
const MEDKIT_CHANCE = 0.9;
...
export default function ReactionRush() {
  ...
  // Fit-to-screen (slightly smaller visual size; gameplay bounds unchanged)
  useEffect(() => {
    const onResize = () => {
      const W = dims.W, H = dims.H;
      const vw = window.innerWidth; const vh = window.innerHeight;
      const scaleW = vw / W; const scaleH = (vh - 80) / H;
      const SCALE_BIAS = 0.94; // ~6% smaller look
  ...
  // Spawn zones in the attacking third on first possession
  ...
  const resetGame = () => {
    ...
    g.defenders = [];
    for (let i=0;i<6;i++) g.defenders.push(makeDefender(dims, PRESETS[preset]));
    g.defenders.forEach(d => { d._W = dims.W; d._H = dims.H; d.frozenT = 0; });
    g.particles = []; g.trail = []; g.texts = []; g.lastSpawn = 0;
    g.shake = 0; g.freezeT = 0; g.iceCd = ICE_CD_MIN + Math.random()*ICE_CD_VAR;
    g.dash = { ready: true, t: 0, cooldown: false };
    g.balls = [];
    g.powerplayT = 0; g.dynamiteCooldown = 0; g.playerGlowT = 0; g.slowmoT = 0; g.borderPulse = null; g.carryBall = null;
    g.goalT = 0;
    g.medkitCd = MEDKIT_MIN + Math.random()*MEDKIT_VAR;
    g.zones = []; g.boostT = 0; g.shotBuffT = 0;
    g.comboCount = 0; g.comboT = 0; g.comboBest = 0;
    g.difficultySteps = 0;
    setLives(5);
    setCollected(0); setTimeLeft(ROUND_TIME); setPaused(false); setRunning(true);
  };
  ...
  useRaf((raw) => {
    ...
    // ----- dynamic difficulty from score -----
    const diff = computeDifficulty(collected);
    const cfg2 = {
      ...cfg,
      baseSpeed: cfg.baseSpeed * diff.speedMul,
      maxDef: cfg.maxDef + diff.extraDef,
      spawnEvery: cfg.spawnEvery / diff.speedMul,
    };
    ...
    // Globals
    g.freezeT = Math.max(0, (g.freezeT || 0) - dt);
    g.iceCd   = Math.max(0, (g.iceCd   || 0) - dt);
    g.powerplayT = Math.max(0, (g.powerplayT || 0) - dt);
    g.dynamiteCooldown = Math.max(0, (g.dynamiteCooldown || 0) - dt);
    g.playerGlowT = Math.max(0, (g.playerGlowT || 0) - dt);
    g.slowmoT = Math.max(0, (g.slowmoT || 0) - dt);
    g.goalT = Math.max(0, (g.goalT || 0) - dt);
    g.shake = Math.max(0, (g.shake || 0) - dt * 0.035);
    g.medkitCd = Math.max(0, (g.medkitCd || 0) - dt);
    g.boostT = Math.max(0, (g.boostT || 0) - dt);
    g.shotBuffT = Math.max(0, (g.shotBuffT || 0) - dt);
    g.comboT = Math.max(0, (g.comboT || 0) - dt);
    if (g.comboT === 0 && g.comboCount > 0) g.comboCount = 0;

    // Zones life
    g.zones.forEach(z => { z.life -= dt; z.pulse += dt; });
    g.zones = g.zones.filter(z => z.life > 0 && !z.used);

    if (g.borderPulse) { g.borderPulse.t = Math.max(0, g.borderPulse.t - dt); if (g.borderPulse.t <= 0) g.borderPulse = null; }

    // Time scale for movement only
    ...
    if (g.lastSpawn > cfg2.spawnEvery) {
      g.lastSpawn = 0; const spawnX = rand(dims.W * 0.2, dims.W * 0.95), spawnY = rand(50, dims.H - 50);
      if (g.iceCd <= 0 && Math.random() < 0.5) {
        g.iceCd = ICE_CD_MIN + Math.random()*ICE_CD_VAR;
        g.balls.push({ x: spawnX, y: spawnY, r: 11, type: 'ice', value: 1 });
      } else {
        const roll = Math.random();
        if (roll < 0.10) g.balls.push({ x: spawnX, y: spawnY, r: 12, type: 'gold', value: 0 });
        else if (roll < 0.18) g.balls.push({ x: spawnX, y: spawnY, r: 12, type: 'powerplay', value: 0 });
        else if (roll < 0.24) g.balls.push({ x: spawnX, y: spawnY, r: 13, type: 'dynamite', value: 0 });
        else if (roll < 0.40) g.balls.push({ x: spawnX, y: spawnY, r: 9,  type: 'dribble', value: 0 });
        else {
          const r2 = Math.random();
          const spec = r2 < 0.55 ? { r: 7, value: 1 } : r2 < 0.85 ? { r: 10, value: 2 } : { r: 13, value: 3 };
          g.balls.push({ x: spawnX, y: spawnY, ...spec, type: 'normal' });
        }
      }
    }

    // Medkit spawn (survival)
    if (gameMode === 'survival' && g.medkitCd <= 0) {
      g.medkitCd = MEDKIT_MIN + Math.random()*MEDKIT_VAR;
      if (lives < 5 && Math.random() < MEDKIT_CHANCE) {
        const mx = rand(dims.W * 0.15, dims.W * 0.85), my = rand(60, dims.H - 60);
        g.balls.push({ x: mx, y: my, r: 13, type: 'medkit', value: 0 });
      }
    }

    // Defenders (scaled + add when tiers increase)
    const targetMax = cfg2.maxDef;
    if ((g.difficultySteps || 0) < diff.extraDef) {
      for (let i = (g.difficultySteps || 0); i < diff.extraDef; i++) {
        if (g.dynamiteCooldown === 0) g.defenders.push(makeDefender(dims, cfg2));
      }
      g.difficultySteps = diff.extraDef;
    }
    if (g.defenders.length < targetMax && g.dynamiteCooldown === 0 && Math.random() < 0.003) {
      g.defenders.push(makeDefender(dims, cfg2));
    }
    if (g.dynamiteCooldown === 0 && g.defenders.length === 0) {
      for (let i = 0; i < targetMax; i++) g.defenders.push(makeDefender(dims, cfg2));
    }
    ...
    // Player physics
    const PLAYER_MAX_SPEED = 1.95; const PLAYER_ACCEL = 0.0105; const PLAYER_FRICTION = 0.0032; const DASH_SPEED = 3.0; const DASH_DURATION = 420;
    let dash = g.dash;
    const inputX = ((rt ? 1 : 0) - (lf ? 1 : 0));
    const inputY = ((dn ? 1 : 0) - (up ? 1 : 0));
    p.vx += inputX * PLAYER_ACCEL * dtMove; p.vy += inputY * PLAYER_ACCEL * dtMove;
    const hasInput = inputX !== 0 || inputY !== 0;
    const reversingX = inputX && Math.sign(inputX) !== Math.sign(p.vx);
    const reversingY = inputY && Math.sign(inputY) !== Math.sign(p.vy);
    let frMult = 1.0; if (!hasInput) frMult = 1.35; if (reversingX || reversingY) frMult = 1.9;
    const spdArc = Math.hypot(p.vx, p.vy);
    if (spdArc > 0) {
      const decay = Math.max(0, 1 - (PLAYER_FRICTION * frMult) * dtMove); p.vx *= decay; p.vy *= decay;
    }
    ...
    // Trail
    if (Math.hypot(p.vx, p.vy) > 0.6 || dashing) {
      g.trail.push({ x:p.x, y:p.y, r:p.r, a:0.6 });
      if (g.trail.length > 14) g.trail.shift();
    }
    g.trail.forEach(t => t.a = Math.max(0, t.a - dt/600));

    // Defenders (scaled cfg)
    g.defenders.forEach((d) => {
    ...
    if (shootPressed && g.carryBall && g.carryBall.state==='carried'){
      ...
      const norm = Math.hypot(dirx, p.vy) || 1;
      g.carryBall = { state:'shot', x:p.x, y:p.y, r:7, vx:(dirx/norm)*0.7, vy:(p.vy/norm)*0.7, life:2200 };
    }
        ...
        if (p.invuln<=0 && g.powerplayT<=0) {
          for (const d of g.defenders){
            if (dist2(d,p) < (((d.r * PRESETS[preset].hitboxScale) + p.r) ** 2)) {
              g.balls.push({ x: cb.x, y: cb.y, r: 9, type:'dribble', value:0 });
              g.carryBall = null;
              g.texts.push({ x:p.x, y:p.y-12, text:'TACKLED!', col:'#fca5a5', life:800 });
              break;
            }
          }
        }
    ...
    // Pickups (combos increment here)
    g.balls = g.balls.filter((b) => {
      if (dist2(b, p) < (((b.r * PRESETS[preset].pickupMagnet) + p.r) ** 2)) {
        const base = b.value || 1; const bonus = (b.type === 'gold') ? 5 : 0;
        const eligibleForCombo = ['normal','gold','ice','powerplay','dynamite','dribble','medkit'].includes(b.type);

        // combo bookkeeping
        if (eligibleForCombo) {
          const prev = g.comboCount;
          g.comboCount = prev + 1;
          g.comboT = COMBO_WINDOW;
          g.comboBest = Math.max(g.comboBest, g.comboCount);
          const inc = comboBonus(g.comboCount) - comboBonus(prev);
          if (inc > 0) {
            setCollected(c=>c+inc);
            g.texts.push({ x:b.x, y:b.y-16, text:`${(COMBO_TIERS.find(t=>t.n<=g.comboCount) || COMBO_TIERS[0]).label} +${inc}`, col:'#38bdf8', life:900 });
          } else {
            g.texts.push({ x:b.x, y:b.y-16, text:`x${g.comboCount}`, col:'#60a5fa', life:650 });
          }
        }
        ...
        if (b.type === 'ice') {
          g.freezeT = FREEZE_DURATION; spawnBurst(g, b.x, b.y, '#93c5fd');
          g.borderPulse = { t: 450, dur: 450, color: '#93c5fd' };
          g.texts.push({ x:b.x, y:b.y, text:'FREEZE!', col:'#93c5fd', life:900 });
        } else if (b.type === 'gold') {
          spawnCoinBurst(g, b.x, b.y); g.playerGlowT = 1000;
          g.borderPulse = { t: 400, dur: 400, color: '#fbbf24' };
          g.texts.push({ x:b.x, y:b.y, text:'+5 XP', col:'#fbbf24', life:900 });
        } else if (b.type === 'powerplay') {
          g.powerplayT = POWERPLAY_DURATION; g.slowmoT = 500; spawnWebPulse(g, b.x, b.y);
          g.borderPulse = { t: 650, dur: 650, color: '#f472b6' };
          g.texts.push({ x:b.x, y:b.y, text:'POWERPLAY!', col:'#f472b6', life:900 });
        } else if (b.type === 'dynamite') {
          g.dynamiteCooldown = DYNAMITE_RESPAWN_DELAY; spawnShockwave(g, b.x, b.y, '#f87171', 6, 700);
          g.borderPulse = { t: 700, dur: 700, color: '#f87171' };
          g.texts.push({ x:b.x, y:b.y, text:'BOOM!', col:'#f87171', life:1000 });
          g.defenders.forEach(d => spawnDefenderGibs(g, d)); g.defenders = [];
        } else if (b.type === 'dribble') {
          g.carryBall = { state:'carried', x:b.x, y:b.y, r:7 };
          g.texts.push({ x:b.x, y:b.y, text:'POSSESSION!', col:'#22c55e', life:900 });
          if (g.zones.length < 1) spawnZonesForPossession(g);
        } else if (b.type === 'medkit') {
          setLives(v => Math.min(5, v + 1));
          spawnBurst(g, b.x, b.y, '#34d399');
          g.borderPulse = { t: 500, dur: 500, color: '#34d399' };
          g.texts.push({ x:b.x, y:b.y, text:'LIFE +1', col:'#34d399', life:900 });
        } else {
          spawnBurst(g, b.x, b.y, '#ffcc00');
          g.texts.push({ x:b.x, y:b.y, text:`+${base}`, col:'#ffd60a', life:800 });
        }
    ...
    g.zones.forEach(z => {
      if (!z.used && dist2(z, p) < ((z.r + p.r) ** 2)) {
        ...
        if (z.type === 'speed') {
          g.boostT = 3000;
          g.texts.push({ x:z.x, y:z.y, text:'SPEED +', col:'#22d3ee', life:900 });
          g.borderPulse = { t: 500, dur: 500, color: '#22d3ee' };
        } else if (z.type === 'invincible') {
          p.invuln = Math.max(p.invuln, 3000);
          g.texts.push({ x:z.x, y:z.y, text:'INVINCIBLE', col:'#eab308', life:900 });
          g.borderPulse = { t: 500, dur: 500, color: '#eab308' };
        } else if (z.type === 'shot') {
          setCollected(c=>c+3);
          g.shotBuffT = 3000;
          g.texts.push({ x:z.x, y:z.y, text:'SHOT ZONE +3', col:'#a78bfa', life:900 });
          g.borderPulse = { t: 500, dur: 500, color: '#a78bfa' };
        }
      }
    });

    // Defender collision
    let hit = false; let hitIdx = -1;
    if (p.invuln <= 0) {
      for (let i = 0; i < g.defenders.length; i++) {
        const d = g.defenders[i];
        if (dist2(d, p) < (((d.r * PRESETS[preset].hitboxScale) + p.r) ** 2)) { hit = true; hitIdx = i; break; }
      }
    }
    if (hit) {
      p.invuln = 800; g.shake = Math.min((g.shake || 0) + 6, 14);
      g.comboCount = 0; g.comboT = 0;
      if (g.powerplayT > 0 && hitIdx >= 0) {
        const d = g.defenders[hitIdx]; d.frozenT = 5000; spawnBurst(g, d.x, d.y, '#a78bfa', 20);
        g.texts.push({ x:d.x, y:d.y, text:'WEBBED!', col:'#a78bfa', life:800 });
      } else {
        setLives(prev => {
          const next = Math.max(0, prev - 1);
          if (next <= 0 && gameMode === 'survival') setTimeout(() => setRunning(false), 0);
          return next;
        });
    ...
    // End conditions
    if (gameMode === 'timed' && (timeLeft - dt <= 0)) {
      setRunning(false); setPaused(false); setBest((b)=>Math.max(b, collected));
    }

    // Render
    drawWorld(canvasRef.current, dims, g, p);
  }, running && !paused);
        ...
        <div style={{ width:'100%', maxWidth:1100 }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12, gap:8, flexWrap:'wrap' }}>
            ...
            <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
              {!running ? btn('Start', resetGame) : (<>
                {btn(paused ? 'Resume' : 'Pause', ()=>setPaused(p=>!p), 'secondary')}
                {btn('Restart', resetGame, 'secondary')}
              </>)}
          ...
          {/* Mode select (only when not running) */}
          {!running && (
              ...
              {['timed', 'survival'].map(m => (
                <button
                  ...
                  style={{
                    ...
                    background: gameMode === m ? '#0b3b1a' : 'transparent',
                    color: '#fff',
                    cursor: 'pointer'
                  }}
                >
              ...
              <div style={{ flex:1 }}>
                ...
                {gameMode === 'survival' && (
                  <div style={{ display:'flex', alignItems:'center', gap:6, fontSize:14 }}>
                    <span>Lives:</span>
                    <span style={{ letterSpacing: 2 }}>{'❤️'.repeat(lives) || '—'}</span>
                  </div>
                )}
              </div>
