...
/** Aurora (Fortnite-style) page background */
function RRPageBackground() {
  return (
    <>
      <style>{`
        .rr-bg { position: fixed; inset: 0; z-index: 0; pointer-events: none; overflow:hidden; }
        .rr-bg.aurora {
          background:
            radial-gradient(120% 80% at 50% 0%, #4338ca 0%, rgba(67,56,202,0.6) 30%, rgba(67,56,202,0.15) 55%, rgba(2,6,23,0.9) 100%),
            linear-gradient(180deg, #0ea5e9 0%, #7c3aed 45%, #0ea5e9 90%);
        }
        .rr-stars { position:absolute; inset:-10%; background-image:
          radial-gradient(1px 1px at 20% 20%, rgba(255,255,255,0.9) 50%, rgba(255,255,255,0) 51%),
          radial-gradient(1px 1px at 80% 30%, rgba(255,255,255,0.9) 50%, rgba(255,255,255,0) 51%),
          radial-gradient(1px 1px at 60% 70%, rgba(255,255,255,0.9) 50%, rgba(255,255,255,0) 51%),
          radial-gradient(1px 1px at 40% 50%, rgba(255,255,255,0.9) 50%, rgba(255,255,255,0) 51%),
          radial-gradient(1px 1px at 25% 75%, rgba(255,255,255,0.9) 50%, rgba(255,255,255,0) 51%);
          opacity:0.35; animation: twinkle 12s ease-in-out infinite;
        }
        .rr-aurora { position:absolute; left:-20%; right:-20%; top:-10%; bottom:-20%; filter: blur(28px); opacity:0.65;
          background:
            conic-gradient(from 0deg at 30% 40%, rgba(59,130,246,0.0), rgba(59,130,246,0.25), rgba(168,85,247,0.22), rgba(59,130,246,0.25), rgba(59,130,246,0.0)),
            conic-gradient(from 180deg at 70% 60%, rgba(34,197,94,0.0), rgba(34,197,94,0.2), rgba(14,165,233,0.18), rgba(34,197,94,0.2), rgba(34,197,94,0.0));
          animation: auroraShift 24s ease-in-out infinite alternate;
        }
        .rr-fog { position:absolute; inset:-10% -20% -20% -20%; background:
          radial-gradient(40% 20% at 20% 40%, rgba(255,255,255,0.12), rgba(255,255,255,0)),
          radial-gradient(30% 15% at 80% 30%, rgba(255,255,255,0.10), rgba(255,255,255,0));
          filter: blur(30px); opacity:0.45; animation: drift 60s ease-in-out infinite alternate;
        }
        @keyframes twinkle { 0%,100%{opacity:0.25} 50%{opacity:0.45} }
        @keyframes auroraShift { 0%{ transform: translateY(-4%) scale(1.02) } 100%{ transform: translateY(4%) scale(1.05) } }
        @keyframes drift { 0%{ transform: translateX(-6%) } 100%{ transform: translateX(6%) } }
      `}</style>

      <div className="rr-bg aurora" aria-hidden="true">
        <div className="rr-stars" />
        <div className="rr-aurora" />
        <div className="rr-fog" />
      </div>
    </>
  );
}
...
// ---------- Mobile controls (outside the canvas) ----------
function useMobileControls() {
  ...
  const UI = !showMobile ? null : (
    <>
      <div onPointerDown={start} onTouchStart={start} onMouseDown={start} style={{ position:'fixed', left:12, bottom:12, width:140, height:140, borderRadius:10, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.12)', touchAction:'none', userSelect:'none', WebkitUserSelect:'none', WebkitTouchCallout:'none', WebkitTapHighlightColor:'transparent', zIndex:1200 }}>
      ...
      <div style={{ position:'fixed', right:12, bottom:12, display:'flex', gap:10, zIndex:1200 }}>
...
// -------------- Error-boundary for NeonIconBar (prevents auth fetch errors from breaking page) --------------
class BarBoundary extends React.Component {
  constructor(props){ super(props); this.state = { err: null }; }
  static getDerivedStateFromError(err){ return { err }; }
  componentDidCatch(){ /* optional: log */ }
  render(){
...
export default function ReactionRush() {
  ...
  const view3D = false; // flat render
  ...
  // Fit canvas (slightly smaller than full); leave headroom so the bar doesn't force scroll
  useEffect(() => {
    const onResize = () => {
      const W = dims.W, H = dims.H;
      const vw = window.innerWidth, vh = window.innerHeight;
      const headroom = 90; // space for NeonIconBar + controls
      const scaleW = vw / W, scaleH = (vh - headroom) / H;
  ...
  useRaf((raw) => {
    ...
    const shootPressed = shootTouch || k.has(' '); // <-- single declaration; no duplicates
    ...
    drawWorld(canvasRef.current, dims, g, p, { view3D });
  }, running && !paused);
  ...
  return (
    <>
      ...
      {/* Compact top bar with NeonIconBar (centered, tight spacing; won’t push canvas down) */}
      <div style={{ position:'relative', zIndex: 5, paddingTop: 6, paddingBottom: 6, display:'flex', justifyContent:'center' }}>
        <BarBoundary>
          <NeonIconBar current="home" />
        </BarBoundary>
      </div>
      ...
      <div style={{ position:'relative', zIndex:1, minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:16, background:'transparent', color:'#e2e8f0' }}>
        <div style={{ width:'100%', maxWidth:1100 }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center', justifyContent:'center', flexWrap:'wrap', marginBottom:10 }}>
            ...
            {!running && (
              <>
                <span style={{ fontSize:12, opacity:0.8, marginLeft:6 }}>Mode:</span>
          ...
          {/* A tiny spacer for ultra-short viewports */}
          <div aria-hidden style={{ height:'6vh' }} />
        </div>
      </div>

      {/* Mobile controls live outside the canvas and won’t overlap it */}
      {mobile.UI}
    </>
  );
}
